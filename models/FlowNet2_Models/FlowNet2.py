...
class FlowNet2(nn.Module):
    ...
    def forward(self, img1, img2):
        ...
        ### normaize input
        # img1.size() = torch.Size([1, 3, 512, 832])
        sz = img1.size()
        img1 = img1.view(sz[0], sz[1], 1, sz[2], sz[3] )
        img2 = img2.view(sz[0], sz[1], 1, sz[2], sz[3] )
        
        # inputs.size() = torch.Size([1, 3, 2, 512, 832]) 
        # 两张图片横向拼接在一起。
        inputs = torch.cat((img1, img2), dim=2)
        
        # rgb_mean.size() = torch.Size([1, 3, 1, 1, 1]) 
        # 把两张图片像素值连在一起算出均值，再分配给最后三个维度。.contiguous()保证了view之后的语义连续性。
        rgb_mean = inputs.contiguous().view(inputs.size()[:2]+(-1,)).mean(dim=-1).view(inputs.size()[:2] + (1,1,1,))
        # print("rgb_mean.size=",rgb_mean.size())
        
        
        # 均值归一化
        x = (inputs - rgb_mean) / self.rgb_max
        # x.size() = torch.Size([1, 3, 2, 512, 832]) 
        x1 = x[:,:,0,:,:]
        x2 = x[:,:,1,:,:]
